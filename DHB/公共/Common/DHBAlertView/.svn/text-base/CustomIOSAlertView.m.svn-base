...
const static CGFloat kCustomIOSAlertViewDefaultButtonSpacerHeight = 20;
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 290, 150)];
    }
    ...
    dialogContainer.layer.borderColor = HEX_RGB(0x757575).CGColor;
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    CGFloat buttonWidth = ((container.bounds.size.width-40)-([buttonTitles count]-1)*20) / [buttonTitles count];

    for (int i=0; i<[buttonTitles count]; i++) {

        DHBButton *closeButton = [[DHBButton alloc] initWithFrame:CGRectMake(20+i * buttonWidth+i*20, container.bounds.size.height - buttonHeight-buttonSpacerHeight, buttonWidth, buttonHeight) style: (DHBButton_Style)[[[buttonTitles objectAtIndex:i] objectForKey:@"type"] integerValue]];

        [closeButton addTarget:self action:@selector(customIOS7dialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        [closeButton setTag:i];

        [closeButton setTitle:[[buttonTitles objectAtIndex:i] objectForKey:@"name"] forState:UIControlStateNormal];
...
- (CGSize)countScreenSize
{
    if (buttonTitles!=NULL && [buttonTitles count] > 0) {
        buttonHeight       = kCustomIOSAlertViewDefaultButtonHeight*Multiple;
        buttonSpacerHeight = kCustomIOSAlertViewDefaultButtonSpacerHeight*Multiple;
    } else {
        buttonHeight = 0;
