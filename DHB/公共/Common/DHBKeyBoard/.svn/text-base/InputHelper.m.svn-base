...
- (void)initilize
{
    ...
    _defaultMaskViewFrame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    _toolBarFrameInMaskView = CGRectMake(0, SCREEN_HEIGHT - 40, SCREEN_WIDTH, 40);
    ...
    [self.toolBarForCleanMaskView removeFromSuperview];
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:DidClickButtonDone object: [self getFirstResponder]];
}
...
- (void)checkText:(NSString *)text forInputField:(UIView *)inputField validation:(NSString *)validation{
    ...
    NSLog(@"current text %@ len %ld,prelen %ld",text,(long)[text length], (long)preLength);
