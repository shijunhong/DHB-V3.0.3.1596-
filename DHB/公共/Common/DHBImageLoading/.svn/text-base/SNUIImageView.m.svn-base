...
- (void)setUp
{
    self.placeholderImage = [UIImage imageNamed:@"invalid"];
...
- (UIImageView *)topLineImage {
    if (!_topLineImage) {
        ...
        _topLineImage.image = [UIImage streImageNamed:@"line"];
...
- (UIImageView *)rightLineImage {
    if (!_rightLineImage) {
        ...
        _rightLineImage.image = [UIImage streImageNamed:@"segment_vertical_line"];
...
- (UIImageView *)bottomLineImage {
    if (!_bottomLineImage) {
        ...
        _bottomLineImage.image = [UIImage streImageNamed:@"line"];
...
- (UIImageView *)leftLineImage {
    if (!_leftLineImage) {
        ...
        _leftLineImage.image = [UIImage streImageNamed:@"segment_vertical_line"];
...
- (void)setImageURL:(NSURL *)imageURL
{
    if (_imageURL != imageURL) {
        ...
        [self sd_setImageWithURL:_imageURL placeholderImage:self.placeholderImage options:self.cacheOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            if (self.shouldShowIndicator) [weakSelf.indicatorView stopAnimating];
...
- (void)setImage:(UIImage *)image
{
    if (image == self.placeholderImage)
    {
        if (!_adjustedPlaceholder) {
            _adjustedPlaceholder = [self adjustPlaceholderImage:image size:self.frame.size];
        }
...
- (void)addRightBottomLine {
    ...
    horizonImageView.image = [UIImage streImageNamed:@"line"];
    ...
    verticalImageView.image = [UIImage streImageNamed:@"segment_vertical_line"];
...
- (void)addTopRightBottonLine {
    ...
    topImageView.image = [UIImage streImageNamed:@"line"];
    ...
    rightImageView.image = [UIImage streImageNamed:@"segment_vertical_line"];
    ...
    bottomImageView.image = [UIImage streImageNamed:@"line"];
...
- (void)addFullLine {
    ...
    leftImageView.image = [UIImage streImageNamed:@"segment_vertical_line"];
...
- (void)setUp
{
    ...
    [self setTouchEndBlock:^(SNUIImageView *imageView) {
        ...
        if ([weakSelf.exDelegate respondsToSelector:@selector(imageExViewDidOk:)]) {
            [weakSelf.exDelegate imageExViewDidOk:weakSelf];
        }
    }];
}
