...
-(NSMutableArray *)buttonss{
    if (!_buttonss)
    {
        _buttonss = [NSMutableArray array];
    }
    return _buttonss;
}
                ...
                resourceDelegate:(NSObject <CustomTabBarResourceDelegate>*)customTabBarResourceDelegate
{
    if (self = [super init])
    {
      ...
      //  _position = BadgePositionTopRight;
        ...
        //tabbar 大背景
        UIImageView* iv_tabbar_bg = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, scrollView.frame.size.width, scrollView.frame.size.height)];
        [iv_tabbar_bg setImage:[UIImage imageNamed:@"pcb_tabbar_bg"]];
        [scrollView insertSubview:iv_tabbar_bg atIndex:0];
        
        CGFloat horizontalOffset = 0;
        [self.buttonss removeAllObjects];
...
//            [newButton addTarget:self action:@selector(otherTouchesAction:) forControlEvents:UIControlEventTouchDragInside];
            ...
            newButton.imageEdgeInsets = UIEdgeInsetsMake(6.0f, 0.0f, labelFrame.size.height + 4.0f, 0.0f);
           ...
           // NSLog(@"newLabel=%@",[NSValue valueWithCGRect:newLabel.frame]);
            ...
            [self.buttonss addObject:newButton];
        }
...
- (void)toggleTabBar:(Boolean)isShow animated:(Boolean)animated {
    ...
    if (animated ) {
		...
		[UIView beginAnimations:nil context:( void *)[NSNumber numberWithBool:isShow]]; {
