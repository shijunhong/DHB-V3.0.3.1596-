//
//  PaymentsDetailsTableViewCell.m
//  DHB
//
...
#import "DHBPaymentsDetailsTableViewCell.h"

@implementation DHBPaymentsDetailsTableViewCell
...
- (void)addFloor_1{
    DHBPaymentsInfoModuleDTO *dto = [[_floorDTO moduleList] firstObject];
    //状态
    ...
    if([[dto amount] doubleValue] > 0){
        priceColor = HEX_RGB(0x1bb635);
        priceStr = [NSString stringWithFormat: @"+%@ 元",[dto amount]];
    }
    else if([[dto amount] doubleValue] == 0){
        priceColor = [UIColor TextBlack_Color];
        priceStr = [NSString stringWithFormat: @"%@ 元",[dto amount]];
    }
    else{
        priceColor = HEX_RGB(0xf70d1a);
        priceStr = [NSString stringWithFormat: @"%@ 元",[dto amount]];
    }
    NSMutableAttributedString *attributteString = [[NSMutableAttributedString alloc] initWithString: priceStr];
    ...
    [attributteString addAttribute: NSForegroundColorAttributeName value: priceColor range: NSMakeRange(0, [attributteString length] - 2)];
    [attributteString addAttribute: NSForegroundColorAttributeName value: priceColor range: NSMakeRange([attributteString length] - 2, 2)];
...
- (void)addFloor_2{
    ...
    [typeValueLabel setText: [dto deposit_type]];
    ...
    [payValueLabel setText: [dto type_id]];
    ...
    [linkOrderValueLabel setText: [dto orders_num]];
    ...
    [payTimeValueLabel setText: [dto receipts_date]];
    ...
    [receiveTimeValueLabel setText: [dto update_date]];
    ...
    [accountLabel sd_layout].leftSpaceToView(self.contentView, MARGIN_LEFT).topSpaceToView(receiveTimeLabel, 12.0f).widthIs((SCREEN_WIDTH - 2 * MARGIN_LEFT) / 2.0f).autoHeightRatio(0);
    UILabel *accountValueLabel = [[UILabel alloc] init];
    [accountValueLabel setTextColor: [UIColor TextGray_Color]];
    [accountValueLabel setText: [dto account_number]];
    ...
    [accountValueLabel sd_layout].topEqualToView(accountLabel).rightSpaceToView(self.contentView, MARGIN_LEFT).widthIs((SCREEN_WIDTH - 2 * MARGIN_LEFT) / 2.0f).autoHeightRatio(0);
    
    ///收款银行
    UILabel *bankLabel = [[UILabel alloc] init];
    [bankLabel setText: [dto bank_name]];
    ...
    [nameLabel setText: [dto account_name]];
...
- (void)addFloor_3{
    ...
    [remarkstextField setText: [dto remark]];
...
- (void)addFloor_4{
    ...
    EGOImageView *voucherImageView = [[EGOImageView alloc] init];
    [voucherImageView setImageURL: [NSURL URLWithString: [dto resource]]];
    [self.contentView addSubview: voucherImageView];
    [voucherImageView sd_layout].leftSpaceToView(self.contentView, MARGIN_LEFT).topSpaceToView(voucherLabel, MARGIN_LEFT).rightSpaceToView(self.contentView, MARGIN_LEFT).heightIs([[dto resource] length] > 0 ? SCREEN_WIDTH - 2 * MARGIN_LEFT : 0);
    
    /////底部section
    UIView *bottomView = [[UIView alloc] init];
    [bottomView setBackgroundColor: [UIColor UIViewBackGroundColor]];
    [self.contentView addSubview: bottomView];
    [bottomView sd_layout].topSpaceToView(voucherImageView, 15.0f).leftSpaceToView(self.contentView, 0.0f).rightSpaceToView(self.contentView, 0.0f).heightIs(30.0f);
    //分割线
    UIView *lineView = [[UIView alloc] initWithFrame: CGRectMake(0.0f, 0.0f, SCREEN_WIDTH, DEFAULT_LINE_HEIGHT)];
    [lineView setBackgroundColor: HEX_RGB(0xd8d8d8)];
    [bottomView addSubview: lineView];
